...
# Конфигурация
DATA_PATH = "data/application.csv"  # путь к CSV с данными
TARGET_COL = "target"  # имя целевого столбца
TEST_SIZE = 0.2
RANDOM_STATE = 42

MODELS_DIR = "models/first_best"
IMAGES_DIR = "images/metrics/first"
...
def save_best_artifacts(
    ...
    """Сохраняем лучшую модель и графики.

    Args:
        results (Dict[str, Dict[str, Any]]): Mapping model_name -> {estimator, proba, metrics, ...}.
        preprocessor (ColumnTransformer): Fitted preprocessor.
        y_test (np.ndarray): Test labels.
        models_dir (str): Directory to save joblib artifacts.
        images_dir (str): Directory to save plots.
        threshold (float): Threshold for confusion matrix.

    Returns:
        Dict[str, Any]: Saved artifacts metadata (best model name, metrics, paths).
    """
    ...
    # Also store a small summary JSON
    summary = {
